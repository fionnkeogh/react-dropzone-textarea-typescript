...
function DropzoneTextArea ({
    ...
    colorActive,
    classNameIsActive, 
}: InferProps<typeof DropzoneTextArea.propTypes>) {
    const onDropRejected = useCallback((files: any[]) => {
      // console.error(files, event);
      const errors = files.map((file) => file.errors);
      const message: string = errors[0].map((error: { message: string }) => error.message).join('. ');
      onError(message);
    },[]);
    ...
    const onDrop = useCallback(
        ...
        reader.onloadend = () => {
            ...
            if (
                ...
                customTextConverter(resultAsArrayBuffer, { file }).then((text: string) => {
                    onDropRead(text);
                });
            }
        };
